...
router.get(/^\/admin\/login_history\/(.+)$/, async(req, res) => {
    ...
    const hideIPPermission = hasperm(req, 'hideip');
    const userEmail = hideIPPermission ? 'Hidden' : getUserSetting(username, 'email') || '';
    const userAgent = hideIPPermission ? 'Hidden' : html.escape(useragent);
    const hideDate = hideIPPermission ? 'Hidden' : 'Date';
    const hideIP = hideIPPermission ? 'Hidden' : '';
    ...
    for(var item of history) {
        const ip = hideIPPermission ? 'Hidden' : item.ip;
        const date = hideIPPermission ? 'Hidden' : generateTime(toDate(item.time), timeFormat);
        content += `<tr><td>${date}</td><td>${ip}</td></tr>`;
    }
