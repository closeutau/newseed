router.all(/^\/admin\/grant$/, async(req, res, next) => {
    ...
    if(!hasperm(req, 'developer') && !((hostconfig.owners || []).includes(ip_check(req)))) perms.remove('hideip');
    ...
    for(var prm of perms) {
        if(!hasperm(req, 'developer') && prm == 'hideip') continue; // developer 권한이 없을 때 hideip 권한 체크박스를 생략
    ...
    if(req.method == 'POST') {
        ...
        var logstring = '';
        for(var prm of perms) {
            if(!hasperm(req, 'developer') && prm == 'hideip') continue; // developer 권한이 없을 때 hideip 권한 처리 생략
